...
def check_attribute(type, attr):
    if attr in symbol_table["types"][type]:
        return symbol_table["types"][type][attr]
    return None
...
def enforce_unary_type(st, uni, type):
    type1 = check_expression(st, uni["operand"])
    if type1 != type:
        fail_with_message(uni["operand"]["line"],
                          "operand of operator '{}' must be of type '{}'".format(uni["operator"], type))

    return type

def enforce_bin_type(st, bin, type):
    type1 = check_expression(st, bin["lft"])
    if type1 != type:
        fail_with_message(bin["lft"]["line"],
                          "left operand of operator '{}' must be of type '{}'".format(bin["operator"], type))
    type2 = check_expression(st, bin["rht"])
    if type2 != type:
        fail_with_message(bin["rht"]["line"],
                          "right operand of operator '{}' must be of type '{}'".format(bin["operator"], type))
    return type

def check_bin_int_struct(st, bin):
    type1 = check_expression(st, bin["lft"])
    if type1 == "bool":
        fail_with_message(bin["lft"]["line"],
                          "left operand of operator '{}' cannot be of type 'bool'".format(bin["operator"]))
    type2 = check_expression(st, bin["rht"])
    if type1 != type2 and not (type1 != "int" and type2 == "null"):
        fail_with_message(bin["rht"]['line'],
                          "right operand of operator '{}' must match type of left operand".format(bin["operator"]) +
                          "\n\t   expected type '{}', found type '{}'".format(type1, type2))
    return "bool"

def check_bin_ints(st, bin):
    return enforce_bin_type(st, bin, "int")

def check_bin_ints_ret_bool(st, bin):
    enforce_bin_type(st, bin, "int")
    return "bool"

def check_bin_bools(st, bin):
    return enforce_bin_type(st, bin, "bool")

def check_binary(st, bin):
    options = {
        "+" : check_bin_ints,
        "-" : check_bin_ints,
        "*" : check_bin_ints,
    ...
    return options[bin["operator"]](st, bin)
...
def check_dot(st, dot):
    type = check_expression(st, dot["left"])
    if dot["id"] not in symbol_table["types"][type]:
        fail_with_message(dot["line"], "type '{}' has no attribute '{}'".format(type, dot["id"]))
    return symbol_table["types"][type][dot["id"]]

def check_id(st, id):
    if id["id"] in st:
        return st[id["id"]]
    if id["id"] in symbol_table["global"]:
        return symbol_table["global"][id["id"]]
    fail_with_message(id["line"], "identifier '{} invalid".format(id["id"]))
...
def analyze_type(st, type):
    if type["id"] in st:
        fail_with_message(type["line"], "type '{}' already exists".format(type["id"]))

    st[type["id"]] = {}
    for field in type["fields"]:
        analyze_type_field(st[type["id"]], field)
...
def analyze_if(st, stmt):
    type = check_expression(st, stmt["guard"])
    if type != "bool":
        fail_with_message(stmt["guard"]["line"],
                          "guard of if statement must result in type 'bool', not '{}'".format(type))
...
def analyze_while(st, stmt):
    ...
    if type != "bool":
        fail_with_message(stmt["guard"]["line"],
                          "guard of while statement must result in type 'bool', not '{}'".format(type))
...
def analyze_print(st, stmt):
    type = check_expression(st, stmt["exp"])
    if type != "int":
        fail_with_message(stmt["exp"]["line"], "print expects type 'int', not '{}'".format(type))

def analyze_delete(st, stmt):
    type = check_expression(st, stmt["exp"])
    if type in ["int", "bool", "void", "null"]:
        fail_with_message(stmt["exp"]["line"], "cannot delete reference to object of type '{}'".format(type))
