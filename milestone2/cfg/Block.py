...
class Block:
    ...
    def get_label(self):
        string = ""
        string += "label %{}".format(self._label)
        return string
    ...
    def get_id(self, id):
        if id in self.locals:
            ...
            if len(self.predecessors) == 0:
                g = utils.get_global(id)
                return g
            pred = [p.get_id(id) for p in self.predecessors]
            return pred[0]
    ...
    def handle_lvalue(self, lvalue):
        if "left" in lvalue:
            ...
            self.add_instruction(inst)
            idx, type = utils.get_struct_index(val.type, lvalue["id"])
    ...
    def handle_return(self, ret):
        if self.func.type != "void":
            ...
            ret_local = Local(".ret", self.func.type)
